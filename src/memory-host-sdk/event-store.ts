...
const memoryHostAssociativeCandidateSchema = memoryHostRecallResultSchema.extend({
  key: z.string(),
  signalCount: memoryHostFiniteNumberSchema,
  source: z.enum(["short-term", "graph", "pykeen"]).optional(),
  provenance: z.string().optional(),
});
const boundedAssociativeCandidatesSchema = z.array(z.unknown()).transform((values, context) => {
  const parsed = z
    .array(memoryHostAssociativeCandidateSchema)
    .safeParse(values.slice(0, MAX_MEMORY_HOST_EVENT_ITEMS));
  if (!parsed.success) {
    context.addIssue({ code: "custom", message: "invalid associative recall candidate" });
    return z.NEVER;
  }
  return { items: parsed.data, truncated: values.length > MAX_MEMORY_HOST_EVENT_ITEMS };
});
const memoryHostEventRecordSchema = z.discriminatedUnion("type", [
  z.looseObject({
    type: z.literal("memory.associative_recall.injected"),
    timestamp: z.string(),
    storageTruncated: z.unknown().optional(),
    seed: z.string(),
    selectedCount: memoryHostFiniteNumberSchema,
    candidates: boundedAssociativeCandidatesSchema,
...
export function normalizeMemoryHostEventRecordForStorage(
  ...
  if (event.type === "memory.associative_recall.injected") {
    const boundText = (text: string, maxBytes = MAX_MEMORY_HOST_EVENT_PATH_BYTES): string => {
    ...
    const candidates = event.candidates.items.map((candidate) => ({
      key: boundText(candidate.key),
      path: boundText(candidate.path),
      startLine: candidate.startLine,
      endLine: candidate.endLine,
      score: candidate.score,
      ...
      ...(candidate.provenance
        ? { provenance: boundText(candidate.provenance, MAX_MEMORY_HOST_EVENT_TEXT_BYTES) }
        : {}),
    }));
    const normalized = {
      type: event.type,
      timestamp: timestamp.value,
      seed: boundText(event.seed),
      selectedCount: event.selectedCount,
      candidates,
      ...(event.sessionKey ? { sessionKey: boundText(event.sessionKey) } : {}),
      ...(event.sessionId ? { sessionId: boundText(event.sessionId) } : {}),
      ...(truncated ? { storageTruncated: true as const } : {}),
    };
    return Buffer.byteLength(JSON.stringify(normalized), "utf8") <= MAX_MEMORY_HOST_EVENT_JSON_BYTES
      ? normalized
