...
function resolveControlUiBasicCredential(req: IncomingMessage): string | undefined {
  ...
  // Browsers remember the gateway secret as the password. Keep the existing
  // username-only form for clients that leave the password field empty.
  return decoded.slice(separator + 1) || decoded.slice(0, separator) || undefined;
}
...
export async function authorizeControlUiReadRequestOrReply(
  ...
  const token =
    resolveControlUiReadAuthToken(params.req, params.allowQueryToken) ??
    resolveControlUiBasicCredential(params.req);
