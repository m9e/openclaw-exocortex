...
export function createToolHookRegistrars(state: PluginRegistryState) {
  ...
  const registerAgentToolResultMiddleware = (
    ...
    const requiresResultReplacement = options?.requiresResultReplacement;
    if (
      requiresResultReplacement !== undefined &&
      typeof requiresResultReplacement !== "function"
    ) {
      reportRegistrationError(
        record,
        "agent tool result replacement requirement must be a function",
      );
      return;
    }
      ...
      scopes: [
        {
          runtimes,
          ...(matcher ? { matcher } : {}),
          ...(requiresResultReplacement ? { requiresResultReplacement } : {}),
        },
      ],
      source: record.source,
      rootDir: record.rootDir,
