...
describe("resolvePluginTools optional tools", () => {
  ...
  it.each(["optional_tool", "optional_t*", "*_tool"])(
    "selects a wildcard manifest owner while projecting only %s matches",
    (allowedName) => {
      const factory = vi.fn(() => [makeTool("optional_tool"), makeTool("optional_other")]);
      setRegistry([
        createNamedToolEntry("optional-demo", ["optional_tool", "optional_other"], {
          optional: true,
          declaredNames: ["optional_*"],
          factory,
        }),
      ]);

      expectResolvedToolNames(resolveOptionalDemoTools([allowedName]), ["optional_tool"]);
      expect(factory).toHaveBeenCalledTimes(1);
    },
  );

  it("projects an exact runtime grant through its wildcard manifest owner", () => {
    const ownerFactory = vi.fn(() => [makeTool("optional_tool"), makeTool("optional_other")]);
    ...
    const tools = resolvePluginTools(
      createResolveToolsParams({
        runtimePluginToolGrant: { pluginId: "optional-demo", toolNames: ["optional_tool"] },
      }),
    );
