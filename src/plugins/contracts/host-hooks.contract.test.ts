...
describe("host-hook fixture plugin contract", () => {
  ...
  it("keeps repeated middleware runtime and matcher scopes paired", async () => {
    ...
    const requiresResultReplacement = (toolName: string) => toolName === "exec";
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
    ...
    expect(registration.scopes?.[0]?.requiresResultReplacement).toBe(requiresResultReplacement);
    expect(registration.scopes?.[1]?.requiresResultReplacement).toBeUndefined();
    const event = {
      toolCallId: "call-1",
      args: {},
