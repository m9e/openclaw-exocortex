...
describe("native result delivery requirements", () => {
  it.each([
    { name: "web_fetch", denied: true },
    { name: "mcp__browser__web_fetch", denied: true },
    { name: "openclawweb_fetch", denied: true },
    { name: "locksmith_future_tool", denied: true },
    { name: "exec_command", denied: false },
    { name: "locksmith_health", denied: false },
  ])("enforces required replacement before executing $name", async ({ name, denied }) => {
    ...
    registry.agentToolResultMiddlewares.push({
      ...
      scopes: [
        {
          runtimes: ["codex"],
          requiresResultReplacement: (tool) =>
            tool === "web_fetch" || (tool.startsWith("locksmith_") && tool !== "locksmith_health"),
        },
      ],
    });
    setActivePluginRegistry(registry);
    const before = vi.fn(async (request: { params: unknown }) => ({
      blocked: false as const,
      params: requireRecord(request.params, "native tool parameters"),
    }));
    ...
    const result = await invokeNativeHookRelay({
      ...
      event: "pre_tool_use",
      rawPayload: { tool_name: name, tool_input: {}, tool_use_id: "call-1" },
    });
    if (denied) {
  ...
  it("admits exact managed catalog ids and does not scan their already transformed post results twice", async () => {
    const registry = createEmptyPluginRegistry();
    const handler = vi.fn(() => undefined);
    registry.agentToolResultMiddlewares.push({
      pluginId: "guard",
      ...
      source: "test",
      runtimes: ["codex"],
      scopes: [{ runtimes: ["codex"], requiresResultReplacement: () => true }],
    });
    setActivePluginRegistry(registry);
    const relay = registerNativeHookRelay({
      provider: "codex",
      ...
      runId: "guard-run",
      preToolUseLoopDetection: false,
      managedToolHookNames: ["openclawweb_fetch"],
    });
    const rawPayload = {
      tool_name: "openclawweb_fetch",
      tool_input: {},
      tool_response: "SANITIZED_CONTENT",
      tool_use_id: "call-1",
    };
    const before = await invokeNativeHookRelay({
      provider: "codex",
      relayId: relay.relayId,
      event: "pre_tool_use",
      rawPayload,
