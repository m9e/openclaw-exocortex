...
function resolveNativeHookRelayToolPolicies(
  registration: Pick<NativeHookRelayPolicy, "config" | "agentId" | "sessionKey">,
...
function nativePreToolUseMayRunToolPolicy(registration: NativeHookRelayPolicy): boolean {
...
export function nativeHookRelayEventHasLocalWork(
  ...
  if (event === "pre_tool_use") {
    ...
    // is no before_tool_call hook, configured tool policy, or loop detector work.
    return (
      hasBeforeToolCallPolicy() ||
      hasAgentToolResultReplacementRequirement("codex") ||
...
async function runNativeHookRelayPreToolUse(params: {
  ...
  const managedResultDelivery = params.registration.managedToolHookNames?.includes(
    params.invocation.toolName ?? "",
  );
  // MCP keeps its namespace for tool policy. Also check the resolved leaf id for
  // result requirements so a guarded tool cannot evade them through an MCP name.
  const mcpLeafName = toolName.startsWith("mcp__") ? toolName.split("__").at(-1) : undefined;
  if (
    !managedResultDelivery &&
    (requiresAgentToolResultReplacement("codex", toolName) ||
      (mcpLeafName && requiresAgentToolResultReplacement("codex", mcpLeafName)))
  ) {
    return params.adapter.renderPreToolUseBlockResponse(
      `Tool "${toolName}" requires guarded result delivery. Use an OpenClaw managed tool; native Codex output cannot be replaced.`,
    );
  }
...
async function runNativeHookRelayPostToolUse(params: {
  ...
  // The managed bridge already applied replacement before returning to Codex.
  const managedResultDelivery = params.registration.managedToolHookNames?.includes(
    params.invocation.toolName ?? "",
  );
  const result =
    !hasToolResultMiddleware || managedResultDelivery
      ? rawResult
