...
export const AGENT_FIELD_HELP: Record<string, string> = {
  ...
  "agents.defaults.embeddedAgent.executionContract":
    'Embedded OpenClaw execution contract: "default" keeps the standard runner behavior, while "strict-agentic" enables non-visible turn recovery for tool-capable embedded runs. GPT-5-family OpenAI runs auto-activate this when unspecified.',
  ...
  "agents.entries.*.embeddedAgent.executionContract":
    'Optional per-agent embedded OpenClaw execution contract override. Set "strict-agentic" to enable non-visible turn recovery for that embedded agent, or "default" to inherit the standard runner behavior.',
