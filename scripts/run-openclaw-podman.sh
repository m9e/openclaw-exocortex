...
OPTIONAL_ENV_ARGS=()
for key in TELEGRAM_BOT_TOKEN LOCKSMITH_INBOUND_TOKEN LOCKSMITH_AGENT_TOKEN; do
  if [[ -n "${!key:-}" ]]; then
    OPTIONAL_ENV_ARGS+=("-e" "$key")
  fi
done
