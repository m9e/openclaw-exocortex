...
Security middleware can additionally declare `requiresResultReplacement(toolName)`
in its registration options. This synchronous, side-effect-free predicate identifies
tools that must deliver the transformed result to the model. Codex routes required
hosted search through managed `web_search` (or disables it when native domain policy
prevents fallback), and its pre-tool relay denies required native tools unless the
exact hook id belongs to the current managed tool catalog. Native post-tool hooks
remain observers and cannot satisfy this requirement. Middleware that only reduces
or annotates output can omit the predicate. Harness owners query active requirements
with `requiresAgentToolResultReplacement(runtime, toolName)` from
