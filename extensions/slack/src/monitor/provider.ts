...
export async function monitorSlackProvider(opts: MonitorSlackOpts = {}) {
  ...
  const clientOptions = resolveSlackWebClientOptions(
    resolveSlackCredentialProxyClientOptions(slackCfg.credentialProxy, account.identity),
    slackDispatcher,
  );
  ...
  // Bolt otherwise lazily sends auth.test with the token in its request body.
  // Resolve identity first so a credential proxy only needs to replace the auth header.
  let botUserId = "";
  let botId = "";
  ...
  const { app, receiver, socketModeLogger } = createSlackBoltApp({
    ...
    slackMode,
    token,
    ...(botId && botUserId ? { botId, botUserId } : {}),
    appToken: slackMode === "socket" ? (appToken ?? undefined) : undefined,
    signingSecret: signingSecret ?? undefined,
    slackWebhookPath,
    clientOptions: clientOptions as Record<string, unknown>,
    // SAFETY: These are WebClient constructor options passed intact to the Socket Mode app client.
