...
type SlackWriteClientCacheOptions = Pick<
  WebClientOptions,
  "slackApiUrl" | "teamId" | "allowAbsoluteUrls"
>;
...
export function createSlackReadClient(token: string, options: WebClientOptions = {}) {
  return new WebClient(
    token,
    resolveSlackReadClientOptions(mergeSlackClientOptions(token, options)),
  );
}
...
export function createSlackStartupAuthClient(token: string, options: WebClientOptions = {}) {
  const resolvedOptions = resolveSlackWebClientOptions(mergeSlackClientOptions(token, options));
...
export function createSlackLookupClient(token: string, options: SlackLookupClientOptions = {}) {
  return new WebClient(
    token,
    resolveSlackLookupClientOptions(mergeSlackClientOptions(token, options)),
  );
}
...
function slackWriteClientCacheKey(token: string, options: SlackWriteClientCacheOptions): string {
  ...
  const absoluteUrlScope = options.allowAbsoluteUrls === false ? ":relative-only" : "";
  return `${tokenKey}${apiScope}${teamScope}${absoluteUrlScope}`;
}
