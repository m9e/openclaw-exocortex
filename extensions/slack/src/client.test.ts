...
beforeAll(async () => {
  ...
  const slackClient = await import("./client.js");
  ({
  ...
  clearSlackWriteClientCacheForTest = slackClient.clearSlackWriteClientCacheForTest;
...
describe("slack web client config", () => {
  ...
  it("limits cached write-client options to routing scopes", () => {
    expectTypeOf<NonNullable<Parameters<typeof getSlackWriteClient>[1]>>().toEqualTypeOf<
      Pick<WebClientOptions, "slackApiUrl" | "teamId" | "allowAbsoluteUrls">
    >();
  });
