...
import {
  maybeResolveAssociativeRecall,
  setAssociativeRecallSamplerForTests,
      ...
      async (event, ctx) => {
        ...
        const handlerPromise = (async () => {
          try {
            ...
            const resolveAssociativeRecall = async (activeSummary: string | null) =>
              activeMemoryAllowed &&
              invocationConfig.mode !== "off" &&
              effectiveAgentId &&
              memorySlot === "memory-core" &&
              deterministicRecallToolName &&
              allowedRecallTools.includes(deterministicRecallToolName) &&
              isPrivateRecallDestination(destinationContext)
                ? await maybeResolveAssociativeRecall({
                    api,
                    ...
                    activeSummary,
                    workspaceDir: resolveAgentWorkspaceDir(liveConfig, effectiveAgentId),
                    assertActive: () => toolAuthority.assertActive(),
                  })
                : [];
            ...
            if (escalationDecision !== "recall") {
              const associativeRecall = await resolveAssociativeRecall(null);
              deadlineController.signal.throwIfAborted();
              toolAuthority.assertActive();
              const associativeContext =
                associativeRecall.length > 0 ? buildPromptPrefix("", associativeRecall) : undefined;
              ...
              const outcomeContext =
                escalationDecision === "no-recall-intent"
                  ...
                  : undefined;
              const prependContext = [laneOneContext, associativeContext, outcomeContext]
                .filter(Boolean)
                .join("\n");
            ...
            deadlineController.signal.throwIfAborted();
            toolAuthority.assertActive();
            const associativeRecall = await resolveAssociativeRecall(result.summary);
            deadlineController.signal.throwIfAborted();
            toolAuthority.assertActive();
            const associativeContext =
              associativeRecall.length > 0 ? buildPromptPrefix("", associativeRecall) : undefined;
            const recallContext = result.summary
              ...
              : result.status === "unavailable"
                ...
                : undefined;
            const prependContext = [laneOneContext, associativeContext, recallContext]
              .filter(Boolean)
              .join("\n");
            return prependContext ? { prependContext } : undefined;
          } catch (error) {
...
const testing = {
  ...
  resetActiveRecallCacheForTests() {
    ...
    resetActiveMemoryTranscriptForTests();
    resetTriggerRecallRunsForTests();
    setAssociativeRecallSamplerForTests(undefined);
  },
  setAssociativeRecallSamplerForTests,
  setMinimumTimeoutMsForTests,
  setSetupGraceTimeoutMsForTests,
