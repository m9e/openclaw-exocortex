...
import {
  ...
  ASSOCIATIVE_RECALL_CONTEXT_HEADER,
...
function formatAssociativeRecallLines(snippets: readonly AssociativeRecallCandidate[]): string[] {
...
function buildMetadata(
  summary: string,
  associativeRecall: readonly AssociativeRecallCandidate[] = [],
): string {
  return [
    ...(summary
      ? [`<${ACTIVE_MEMORY_PLUGIN_TAG}>`, escapeXml(summary), `</${ACTIVE_MEMORY_PLUGIN_TAG}>`]
      : []),
    ...formatAssociativeRecallLines(associativeRecall),
  ].join("\n");
}
...
function buildPromptPrefix(
  ...
  const header =
    associativeRecall.length > 0 ? ASSOCIATIVE_RECALL_CONTEXT_HEADER : ACTIVE_MEMORY_CONTEXT_HEADER;
