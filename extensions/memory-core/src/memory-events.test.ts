...
describe("memory host event journal integration", () => {
  it("persists associative recall provenance through the bounded event journal", async () => {
    const workspaceDir = await createTempWorkspace("memory-core-associative-events-");
      ...
      candidates: [
        {
          ...
          signalCount: 3,
          source: "pykeen",
          provenance: "relation=DEPENDS_ON ".repeat(200),
        },
      ],
    });
    ...
    if (event?.type !== "memory.associative_recall.injected") {
      throw new Error("expected associative recall event");
    }
    expect(event.selectedCount).toBe(1);
    expect(event.candidates[0]?.source).toBe("pykeen");
    expect(event.candidates[0]?.provenance).toContain("relation=DEPENDS_ON");
    expect(event.storageTruncated).toBe(true);
