...
import { getCodexDynamicToolHookNames } from "./dynamic-tool-catalog.js";
import {
...
export async function runCodexAppServerSideQuestion(
  ...
  try {
    ...
    nativeHookRelay =
      options.nativeHookRelay || hasAgentToolResultReplacementRequirement("codex")
        ? registerCodexSideNativeHookRelay({
            ...
            autoApproveMcpTools,
            projectedMcpServers,
            managedToolHookNames: getCodexDynamicToolHookNames(toolBridge.availableSpecs),
            runId: sideRunParams.runId,
...
function resolveCodexSideNativeHookRelayEvents(params: {
  ...
  if (params.configuredEvents?.length) {
    if (
      ...
      !params.configuredEvents.includes("pre_tool_use")
    ) {
      return [...params.configuredEvents, "pre_tool_use"];
    }
...
function registerCodexSideNativeHookRelay(params: {
  ...
  managedToolHookNames: readonly string[];
  runId: string;
  channelId?: string;
  ...
  if (params.options.enabled === false) {
    if (hasAgentToolResultReplacementRequirement("codex")) {
      throw new Error("Tool result protection requires the Codex native pre-tool relay.");
    }
    return undefined;
  }
  return registerNativeHookRelay({
    ...
    managedToolHookNames: params.managedToolHookNames,
    runId: params.runId,
