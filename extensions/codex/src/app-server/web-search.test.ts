...
const { requiresReplacement } = vi.hoisted(() => ({ requiresReplacement: vi.fn(() => false) }));
vi.mock("openclaw/plugin-sdk/agent-harness-runtime", () => ({
  requiresAgentToolResultReplacement: requiresReplacement,
}));
import { resolveCodexWebSearchPlan } from "./web-search.js";
afterEach(() => requiresReplacement.mockReset());

describe("resolveCodexWebSearchPlan", () => {
  it("routes required result replacement through managed search without disabling other native tools", () => {
    requiresReplacement.mockReturnValue(true);
    expect(resolveCodexWebSearchPlan({ nativeToolSurfaceEnabled: true })).toEqual({
      kind: "managed",
      suppressManagedWebSearch: false,
      threadConfig: { "features.standalone_web_search": false, web_search: "disabled" },
    });
    expect(requiresReplacement).toHaveBeenCalledWith("codex", "web_search");
  });

  it("does not bypass native domain restrictions when result replacement requires managed search", () => {
    requiresReplacement.mockReturnValue(true);
    expect(
