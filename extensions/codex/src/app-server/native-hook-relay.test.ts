...
it("retains the required pre-tool gate when configured hook events omit it", () => {
  ...
  registry.agentToolResultMiddlewares.push({
    ...
    source: "test",
    runtimes: ["codex"],
    scopes: [{ runtimes: ["codex"], requiresResultReplacement: (name) => name === "web_search" }],
  });
  setActivePluginRegistry(registry);
  expect(
    resolveCodexNativeHookRelayEvents({
      configuredEvents: ["post_tool_use"],
      appServer: { approvalPolicy: "never" },
    }),
  ).toEqual(["post_tool_use", "pre_tool_use"]);
});
