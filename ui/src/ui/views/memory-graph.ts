...
let memoryGraphSearch = "";
let selectedMemoryGraphNodeId: string | null = null;
...
function buildGraphModel(props: MemoryGraphProps): GraphModel {
  ...
  addUniqueNode(nodes, seen, {
    ...
    metric: props.shortTermCount + props.promotedTotal + (props.wikiMemoryPalace?.totalItems ?? 0),
  });
  ...
  const clusterNodes: GraphNode[] = [
    ...
    },
    {
      ...
      kind: "cluster",
      group: "Clusters",
      detail: `${props.wikiMemoryPalace?.totalItems ?? 0} wiki memory items.`,
      metric: props.wikiMemoryPalace?.totalItems ?? 0,
    },
  ];
  ...
  for (const cluster of props.wikiMemoryPalace?.clusters ?? []) {
...
function selectedNode(layout: GraphLayout): PositionedNode | null {
  return (
    layout.nodes.find((node) => node.id === selectedMemoryGraphNodeId) ?? layout.nodes[0] ?? null
  );
}
...
function renderStats(props: MemoryGraphProps, model: GraphModel): TemplateResult {
  return html`
    <div class="memory-graph-stats" aria-label="Memory graph statistics">
      ...
      <div class="memory-graph-stat">
        <strong>${props.wikiMemoryPalace?.totalItems ?? 0}</strong>
          ...
          ${layout.nodes.map(
            (node) => html`
              <g
                ...
                @click=${() => {
                  selectedMemoryGraphNodeId = node.id;
...
export function renderMemoryGraph(props: MemoryGraphProps): TemplateResult {
  ...
  const detail = selectedNode(layout);
        ...
        <div class="memory-graph-toolbar__filters">
          <label class="memory-graph-field">
            ...
            <select
              ...
              @change=${(event: Event) => {
                ...
                selectedMemoryGraphNodeId = null;
          ...
          <label class="memory-graph-field memory-graph-field--search">
            ...
            <input
              ...
              @input=${(event: Event) => {
                memoryGraphSearch = (event.currentTarget as HTMLInputElement).value;
                props.onRequestUpdate?.();
              }}
            />
          </label>
          ...
          ${model.nodes.length > 1
            ...
            : html`
                <div class="memory-graph-empty">
                  ...
                  <p>
                    Enable memory dreaming or memory-wiki to populate short-term, promoted, and wiki
                    graph nodes.
                  </p>
                </div>
              `}
          ${renderMatches(layout, memoryGraphSearch, props)}
        </div>
